...
cv::Mat EDCircles::drawResult(bool onImage, ImageStyle style)
{
	...
	if (style == ImageStyle::ELLIPSES|| style == ImageStyle::BOTH)
		...
		{
			double degree = (ellipses[i].theta * 180) / M_PI; // convert radian to degree (opencv's ellipse function works with degree)
...
void EDCircles::DetectArcs(vector<LineSegment> lines)
{
	...
	double MIN_ANGLE = M_PI / 30;  // 6 degrees
	double MAX_ANGLE = M_PI / 3;   // 60 degrees

	for (int iter = 1; iter <= 2; iter++) {
		if (iter == 2) MAX_ANGLE = M_PI / 1.9;  // 95 degrees
											  //    if (iter == 2) MAX_ANGLE = M_PI/2.25;  // 80 degrees
		...
		for (int curSegmentNo = 0; curSegmentNo<segmentNos; curSegmentNo++) {
			...
			while (firstLine < stopLine - 1) {
				...
				if (lastLine - firstLine == 1) {
					...
					if (info[firstLine].angle >= M_PI / 12 && info[firstLine].angle <= M_PI / 4 && totalLineLength >= 40 && shorterLen * 2 >= longerLen) {
						specialCase = true;
					} //end-if
...
{
	double prec = M_PI / 16;  // Alignment precision
